...
const mk = mailkite({
  apiKey: process.env.MAILKITE_API_KEY,   // required — pass it explicitly
...
const mk = mailkiteAdapter({ apiKey: process.env.MAILKITE_API_KEY, from: "auth@acme.com" });
export const auth = betterAuth({
...
| `apiKey` | `string` | **Required** (unless `getToken`). No environment fallback — see below. |
...
`apiKey` is now **required**. Earlier versions fell back to
`process.env.MAILKITE_API_KEY`, which read as convenience and behaved as a trap: on any
runtime without a populated `process.env` — Workers, Deno, most edge runtimes — the
plugin looked configured and sent nothing. Because sends are backgrounded and failures
route to `onError`, the app kept working while every auth email silently vanished.

Pass it explicitly: `mailkite({ apiKey: process.env.MAILKITE_API_KEY, from })`. Nothing
else changed.
