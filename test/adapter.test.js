...
test("an env var does NOT satisfy apiKey — the key must be explicit", () => {
  // 0.2.x fell back to process.env.MAILKITE_API_KEY. That read as convenience and
  // behaved as a trap: on any runtime without a populated process.env (Workers,
  // Deno, edge) the plugin looked configured and sent nothing, and because sends
  // are backgrounded with failures routed to onError, the app looked healthy while
  // every auth email vanished. Explicit or it throws.
  const prev = process.env.MAILKITE_API_KEY;
  process.env.MAILKITE_API_KEY = "mk_live_from_the_environment";
  try {
    assert.throws(
      () => mailkite({ from: "a@b.com", fetch: stubFetch() }),
      /apiKey.*required/i,
    );
  } finally {
    if (prev === undefined) delete process.env.MAILKITE_API_KEY;
    else process.env.MAILKITE_API_KEY = prev;
  }
});
